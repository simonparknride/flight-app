...
def build_docx_stream(records, start_dt, end_dt, reg_placeholder):
    ...
    for i, r in enumerate(records):
        ...
        for j, val in enumerate(vals):
            ...
            run = para.add_run(val); run.font.size = Pt(13)
    target = io.BytesIO()
...
def build_labels_stream(records, start_num, reg_placeholder):
    target = io.BytesIO()
...
if uploaded_file:
    ...
    if records_all:
        ...
        if filtered:
            ...
            docx_data = build_docx_stream(filtered, s_dt, e_dt, "")
            col1.download_button("📥 Download DOCX List", docx_data, f"Flight_List_{fn_date}.docx")
            
            pdf_data = build_labels_stream(filtered, label_start, "")
