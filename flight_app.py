...
def parse_raw_lines(lines: List[str]) -> List[Dict]:
    ...
    while i < len(lines):
        ...
        if m and current_date is not None:
            ...
            records.append({'dt': dep_dt, 'time': time_str, 'flight': flight, 'dest': dest_iata, 'reg': reg})
            i += 4; continue
        i += 1
    return records

# --- 3. DOCX 생성 (1-Page 70% 폭 + 7.5pt 적용) ---
def build_docx_stream(records, start_dt, end_dt, is_1page=False):
    ...
    run_h.bold = True; run_h.font.size = Pt(7.5 if is_1page else 16)
    
    table = doc.add_table(rows=0, cols=5); table.alignment = WD_TABLE_ALIGNMENT.CENTER
    tblPr = table._element.find(qn('w:tblPr'))
    tblW = OxmlElement('w:tblW')
    tblW.set(qn('w:w'), '3500' if is_1page else '4000') # 3500=70% 폭
...
# --- 4. PDF 레이블 (에러 원인 setStrokeGray 교정) ---
def build_labels_stream(records, start_num):
    ...
    for i, r in enumerate(records):
        ...
        # [수정] 에러를 일으켰던 부분을 가장 안전한 표준 함수로 교체
        c.setStrokeGray(0.3) 
        c.setLineWidth(0.2)
