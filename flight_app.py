...
st.markdown("""
    ...
    div.stDownloadButton > button:hover {
        background-color: #60a5fa !important; border: 2px solid #60a5fa !important;
    }
...
def build_labels_stream(records, start_num):
    ...
    for i, r in enumerate(records):
        ...
        # 각 라벨의 '바닥 왼쪽'을 (x0, y0)로 설정
        x0 = margin_x + col * (col_w + gutter)
        y0 = h - margin_y - (row + 1) * row_h
        
        # 테두리
        c.setStrokeColorGray(0.8); c.setLineWidth(0.1)
        c.rect(x0, y0 + 2*mm, col_w, row_h - 4*mm)
        
        # 텍스트 배치 (가이드 PDF 정밀 좌표)
        c.setFillColorGray(0)
        
        # 1. 순번 (Top-Left)
        c.setFont('Helvetica-Bold', 12)
        c.drawString(x0 + 5*mm, y0 + row_h - 12*mm, str(start_num + i))
        
        # 2. 날짜 (Top-Right)
        c.setFont('Helvetica', 10)
        c.drawRightString(x0 + col_w - 5*mm, y0 + row_h - 12*mm, r['date_label'])
        
        # 3. 편명 (Center-Top)
        c.setFont('Helvetica-Bold', 32)
        c.drawCentredString(x0 + col_w/2, y0 + row_h - 28*mm, r['flight'])
        
        # 4. 목적지 (Center-Middle)
        c.setFont('Helvetica-Bold', 22)
        c.drawCentredString(x0 + col_w/2, y0 + row_h - 42*mm, r['dest'])
        
        # 5. 시간 (Center-Bottom)
        tdisp = datetime.strptime(r['time'], '%I:%M %p').strftime('%H:%M')
        c.setFont('Helvetica-Bold', 28)
        c.drawCentredString(x0 + col_w/2, y0 + 18*mm, tdisp)
        
        # 6. 기종/등록번호 (Bottom)
        c.setFont('Helvetica', 9)
        c.drawCentredString(x0 + col_w/2, y0 + 8*mm, f"{r['type']}  {r['reg']}".strip())
        
    c.save(); target.seek(0); return target

# --- 4. DOCX 1-Page (7.5pt 적용) ---
    ...
    if recs:
        ...
        if dates:
            ...
            if filtered:
                ...
                with c1: st.download_button("📥 DOCX (Orig)", build_docx_stream(filtered, start_dt, end_dt), f"{fn}_orig.docx")
                with c2: st.download_button("📄 DOCX (1-Page)", build_docx_stream(filtered, start_dt, end_dt, True), f"{fn}_1p.docx")
                with c3: st.download_button("📥 PDF Labels", build_labels_stream(filtered, label_start), f"Labels_{fn}.pdf")
                with c4: st.download_button("📊 Only Flights", pd.DataFrame([r['flight'] for r in filtered]).to_csv(index=False, header=False).encode('utf-8-sig'), f"{fn}.csv")
